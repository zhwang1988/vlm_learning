...
> 打卡写在 `progress/daily-log.md`，勾选写在 `progress/progress-tracker.md`。
...
### Day 6 — 复盘：一次完整的图文推理
- **读**：总结本周讲义，回看自己的打卡笔记
- **写**：整理 `notebooks/02_minivlm_walkthrough.ipynb`，把 Mini-VLM 的 forward 过程可视化（token 序列、attention mask）
